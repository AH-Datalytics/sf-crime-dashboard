...
import { aggregateByDistrict } from "@/lib/measures/crime-measures";
import { COLORS } from "@/lib/constants";

export function CrimeDistrictChart() {
  const { filtered, isLoading } = useCrimeData();

  const data = useMemo(() => aggregateByDistrict(filtered), [filtered]);
  ...
  return (
    <div className="border border-[#e8e8e8] rounded-lg bg-white p-4">
      <p className="text-xs font-mono uppercase tracking-wider text-gray-400 mb-3">Incidents by Police District</p>
