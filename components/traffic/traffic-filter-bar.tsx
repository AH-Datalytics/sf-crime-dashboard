...
import { POLICE_DISTRICTS, TRAFFIC_SEVERITIES } from "@/lib/constants";

export function TrafficFilterBar() {
  const { severity, setSeverity, district, setDistrict, year, setYear, resetFilters } = useTrafficStore();
  const hasFilters = severity || district || year;
    ...
    <div className="sticky top-12 z-50 bg-white border-b border-[#e8e8e8] px-4 py-2.5">
      <div className="max-w-7xl mx-auto flex flex-wrap items-end gap-3">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-mono uppercase tracking-wider text-gray-400">Severity</label>
          <select
            value={severity ?? ""}
            onChange={(e) => setSeverity(e.target.value || null)}
            className="text-sm border border-[#e8e8e8] rounded px-2 py-1.5 bg-white min-w-[200px]"
          >
            <option value="">All Severities</option>
            {TRAFFIC_SEVERITIES.map((s) => <option key={s}>{s}</option>)}
          </select>
        </div>
        <div className="flex flex-col gap-1">
